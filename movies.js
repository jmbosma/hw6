...
window.addEventListener('DOMContentLoaded', async function(event) {
  ...
  // https://api.themoviedb.org/3/movie/now_playing?api_key=e3043c12bc182129e25ae96b55ea6506&language=en-US

    // request the data from our "API"
    let apiKey = 'e3043c12bc182129e25ae96b55ea6506'
  ...
  for(let i = 0; i < numMovies; i++) {
    ...
    //insert the HTML for the banner for each movie in the array 
  ...
  // ⬇️ ⬇️ ⬇️
    document.querySelector(`.movie-${movieId} .watched-button`).addEventListener('click', async function(event){
      ...
      // console.log(`The movie ${movieTitle} was clicked!`)
      
      await db.collection('watched').doc(`${movieId}`).set({})

      //conditional to turn opaque if the watched button is clicked or remove opaqueness after second click to "unwatch"
      if ( document.querySelector(`.movie-${movieId}`).classList.contains('opacity-20')){
        ...
        //not mentioned in rubric but removes movie from database if someone unclicks the watched button 
